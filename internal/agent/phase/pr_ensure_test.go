...
func TestEnsurePR_NotGitRepo(t *testing.T) {
	r := require.New(t)
	result := EnsurePR(context.Background(), nil, t.TempDir(), "", PRAttributionOpts{})
	r.Empty(result.URL)
...
func TestEnsurePR_OnMainBranch(t *testing.T) {
	...
	_, local := initGitRepoWithRemote(t, "main")
	result := EnsurePR(context.Background(), nil, local, "", PRAttributionOpts{})
	r.Empty(result.URL)
	r.Error(result.Error)
...
func TestEnsurePR_ContextCancelled(t *testing.T) {
	...
	result := EnsurePR(ctx, nil, local, "", PRAttributionOpts{})
