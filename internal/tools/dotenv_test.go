...
func TestIsEnvFile(t *testing.T) {
	...
	tests := map[string]struct {
		...
		".env.example":               {path: ".env.example", want: false},
		".env.sample":                {path: ".env.sample", want: false},
		".env.template":              {path: ".env.template", want: false},
		".env.example nested":        {path: "/app/.env.example", want: false},
...
func TestCommandAccessesEnvFile(t *testing.T) {
	...
	tests := map[string]struct {
		...
		"cp .env.example .env":      {command: "cp .env.example .env", want: ".env"},
		"cat .env.example is safe":  {command: "cat .env.example", want: ""},
		"cat .env.template is safe": {command: "cat .env.template", want: ""},
		"cat .env.sample is safe":   {command: "cat .env.sample", want: ""},
